...
if (isset ($_SESSION['loggedin']) && isset ($_SESSION['time']))
{
	...
	$strLogin .= "<h4 id=\"admin_title\">Goodbye!</h4>\n";
	$strLogin .= "<p>You are now logged out.</p>\n";
	$strLogin .= "<p><a href=\"index.php\">Login</a> again, or go to the <a href=\"http://www.steveblackonline.com/\">Steve Black</a> homepage.</p>\n";
	echo $strLogin;
...
else if (isset ($_POST['login']))
{
	...
	if (!isset ($_COOKIE['tries']) || $_COOKIE['tries'] != '0')
	{
		...
		//search for the user.
		$sqlSearch_login = "SELECT * FROM users WHERE USERNAME='$strUsername' AND PASSWORD='$md5Password'";
		...
		if ($arrSearch_login)
		{
			...
			$_SESSION['time'] = time();
			header ('Location: http://www.steveblackonline.com/admin/');
			exit;
		}
		else
		{
